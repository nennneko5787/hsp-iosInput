...
{
    hgio_setinfo( GINFO_EXINFO_ACCEL_X, (HSPREAL)acceleration.x );
    hgio_setinfo( GINFO_EXINFO_ACCEL_Y, (HSPREAL)acceleration.y );
    hgio_setinfo( GINFO_EXINFO_ACCEL_Z, (HSPREAL)acceleration.z );
...
- (void)showSystemKeyboard {
    ...
    // ビューに追加（非表示のまま）
    [self addSubview:tempTextField];  // self.viewではなくselfを使う場合もある
    // フォーカスを当ててキーボードを表示
    [tempTextField becomeFirstResponder];
