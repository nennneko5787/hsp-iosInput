...
static void Object_JumpEvent(HSPOBJINFO *info, int wparam)
{
    ...
    if (wparam != HSPOBJ_NOTICE_CLICK_END)
        return;
...
void OpenSoftwareKeyboard(HSPOBJINFO *info)
{
    [[InputBoxHelper sharedInstance] showKeyboardFor:info];
}
...
int Bmscr::NewHSPObject(void)
{
    ...
    else
    {
        if (objmax)
        {
            for (i = 0; i < objmax; i++)
            {
                if (mem_obj[i].owmode == HSPOBJ_NONE)
                    return i;
            }
        }
        if (objmax >= objlimit)
            throw HSPERR_WINDOW_OBJECT_FULL;
        id = objmax++;
...
void Bmscr::SetButtonImage(int id, int bufid, int x1, int y1, int x2, int y2, int x3, int y3)
{
    ...
    if (obj->func_notice != Object_JumpEvent)
        throw HSPERR_UNSUPPORTED_FUNCTION;
...
int Bmscr::AddHSPObjectLayer(int sizex, int sizey, int layer, int val, int mode, void *callptr)
{
    ...
    lay = layer & 0xff;
    if (lay < HSPOBJ_OPTION_LAYER_MIN)
        lay = HSPOBJ_OPTION_LAYER_MIN;
    if (lay > HSPOBJ_OPTION_LAYER_MAX)
        lay = HSPOBJ_OPTION_LAYER_MAX;

    if ((layer & HSPOBJ_OPTION_LAYER_MULTI) == 0)
    { // 重複登録を検出する
        obj = mem_obj;
